<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class Restaurent extends CI_Controller{
        ...
        $sess_data['last_insert_id']=$this->data_model->insert_restaurent($table_name,$data);
        $this->session->set_userdata($sess_data);
        redirect('admin/dashboard/add_restaurent_feature_info');
       // $this->load->view('admin/restaurent/add_restaurent_feature_info');
        ...
        function cuisine_category_menu_setting(){
                $this->load->view('admin/restaurent/cuisine_category_menu_setting');
        }
        ...
        function cuisine_item_menu_setting(){
                $this->load->view('admin/restaurent/cuisine_item_menu_setting');
        }
        ...
        function category_filter(){
            ...
            $data['filter']=$this->data_model->category_filter($category_name,$table_name);
            $this->load->view('admin/restaurent/cuisine_item_menu_setting', $data);
        }
        function signature_item(){
            $this->load->view('admin/restaurent/add_signature_item');
        }
        ...
        function add_signature_item_by_a_restaurent()
        {
            ...
            if ($this->form_validation->run() === false) {
                $this->load->view('admin/restaurent/add_signature_item_by_a_restaurent',$data);
            } else {
                $table_name = "signature_item";
                ...
                $this->data_model->insert_data_to_database($table_name, $data);
                redirect('admin/food/fatch_signature_item_by_a_restaurent/'.$res_id);
            }
        }
        function view_signature_item(){
            //$table_name = 'signature_item';
            //$column_name = 'res_name';
            $column_value=$this->session->userdata('res_name');
            $data['signature'] = $this->data_model->view_signature_item($column_value);
            $this->load->view('admin/restaurent/view_signature',$data);
        }
        ...
        function fatch_restaurent(){
                $table_name='indi_res_info';
                $data['all_restaurent']=$this->data_model->fatch_all_data($table_name);
                $this->load->view('admin/restaurent/list_restaurent', $data);
        }
        function restaurent_edit(){
            ...
            if(isset($_GET['d_edit']))
            {
                ...
                //echo $id; exit;
                $data['edit_fetch']=$this->data_model->fetch_to_edit($table,$id,$table_id);
                $this->load->view('admin/restaurent/edit_restaurent',$data);
            }
        }

        function presetup_resturent(){
            $table="indi_res_info";
            $data=array(
                'res_name'                  =>$this->input->post('res_name'),
                'res_address'               =>$this->input->post('res_address'),
        ...
        function restaurent_delete(){
            ...
            //echo $res_id; exit;
            $this->data_model->delete_data($table_name,$res_id);
            //$this->load->view('admin/data_operator_dashboard/restaurent/list_restaurent');
            redirect('admin/restaurent/fatch_restaurent');
        }
        public function menu_manage()
        {
                ...
                $data['restaurant'] = $this->data_model->get_a_restaurant($last_insert_id);
                $data['menus'] = $this->data_model->get_menu_for_restaurant($last_insert_id);
                $menu_array = array();
                ...
                $data['results'] = $this->data_model->get_menus();
		$this->load->view('admin/header_link');
		$this->load->view('admin/header');
		$this->load->view('admin/menu');
                $this->load->view('admin/restaurent/menu_management',$data);
        ...
        public function menu_manage_by_a_restaurent()
        {
                ...
                $data['restaurant'] = $this->data_model->get_a_restaurant_by_a_restaurent($last_insert_id);
                $data['menus'] = $this->data_model->get_menu_for_restaurant_by_a_restaurent($last_insert_id);
                $menu_array = array();
                foreach($data['menus'] as $menu)
                {
                    $menu_array[] = $menu->item_id;
                ...
                $data['value'] = $menu_array;
                $data['results'] = $this->data_model->get_menus_by_a_restaurent();
		$this->load->view('admin/header_link');
		$this->load->view('admin/header');
		$this->load->view('admin/menu');
                $this->load->view('admin/restaurent/menu_management_by_a_restaurent',$data);
		$this->load->view('admin/footer');
        ...
        public function add_menu_price()
        {
            ...
            $result = $this->data_model->add_menu_price($restaurant_id,$price,$menu_id);
            if($result)
            {
                $this->session->set_flashdata('success','Price Updated successfully.');
                redirect('admin/restaurent/menu_manage');
            }
            else
            {
        ...
        public function add_menu_price_by_a_restaurent()
        {
                    ...
                    $result = $this->data_model->add_menu_price_by_a_restaurent($restaurant_id,$price,$menu_id);
                    if($result)
                    {
                        $this->session->set_flashdata('success','Price Updated successfully.');
                        
                        redirect('admin/restaurent/menu_manage_by_a_restaurent/'.$restaurant_id);
        ...
        public function change_item_price()
        {
                ...
                if($_POST)
                {
                    ...
                    $result = $this->data_model->update_item_price($restaurant_id,$item_id,$item_price);
                    if($result)
                    {
                        $this->session->set_flashdata('success','Price updated Successfully');
                        redirect($current_url);
        ...
        public function change_item_price_by_a_restaurent()
        {
                if($_POST)
                {
                    ...
                    $result = $this->data_model->update_item_price_by_a_restaurent($restaurant_id,$item_id,$item_price);
                    if($result)
                    {
                        $this->session->set_flashdata('success','Price updated Successfully');
                        redirect($current_url);
        ...
        function search_item(){ 
            ...
            $data['other_menu']=$this->data_model->search_other_menu($match);
            $data['menus'] = $this->data_model->get_menu_for_restaurant($last_insert_id);
            $menu_array = array();
            foreach($data['menus'] as $menu)
            {
           ...
           //print_r($data['value']);exit;  
            $this->load->view('admin/restaurent/restaurent_menu_setting_search',$data);
        }
        ...
        public function search_add_menu_price()   
        {
            ...
            $result = $this->data_model->search_add_menu_price($restaurant_id,$price,$menu_id);
         ...
         public function search_update_menu_price()   
        {
            ...
            $price = $this->input->post('menu_restaurant_relation_price');
            $menu_id = $this->input->post('item_id');
            $result = $this->data_model->search_update_menu_price($menu_restaurant_relation_id,$restaurant_id,$price,$menu_id);
            if($result)
            {
                $this->session->set_flashdata('success','Price Updated successfully.');
                redirect('admin/restaurent/menu_manage');
        ...
        public function get_restaurent()
        {
                ...
                $data['restautrant_details'] = $this->data_model->get_a_restaurant($restuarant_id);
                //print_r($data); exit;
                $this->load->view('admin/restaurent/restaurant_details',$data);
        }
        ...
        public function front_details() 
         {    
		     ...
		     $this->session->set_userdata('res_id',$res_id);  
                       ...
                       $data['res_info']= $this->data_model->front_info($res_id);
                       $data['gallery_info']= $this->data_model->gallery_info($res_id); 
                       $data['show_feature_info']= $this->data_model->show_feature_info($res_id);
					   					   
                       //$data['menus'] = $this->data_operator_data_model->select_menus($res_id,$c_id);
   
                       $data['c_name'] = $this->data_model->c_name($res_id); 
                       $data['res_pic']= $this->data_model->res_pic($res_id);
                      
                       $data['recent_post']=  $this->data_model->recent_post();
                       $data['list_popular_dishes'] = $this->data_model->list_popular_dishes(); 
			 		   
                                          
                       //$data['res_id']= $this->uri->segment(3, 0);  
                       $data['title']="Indichef";
                       $this->load->view('admin/restaurent/details',$data); 
	...
	function restaurent_menu_setting_complate(){
        ...
        function fatch_restaurent_feature_information(){
            $table_name='indi_feature_info';
            $data['all_feature_info']=$this->data_model->fatch_all_data($table_name);
            $this->load->view('admin/restaurent/restaurent_feature_information_detail', $data);
        }
    }
