                                        ...
                                        <?php if($all_signature_item){  ?>
                                            <?php foreach($all_signature_item as $key){?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo anchor('admin/restaurent/add_signature_item_by_a_restaurent/'.$key['res_id'],'Add Signature');?> &nbsp; <?php echo anchor('admin/food/signature_item_edit_by_a_restaurent/'.$key['signature_item_id'],'Edit');?> &nbsp; <?php echo anchor('admin/food/signature_item_delete_by_a_restaurent/'.$key['signature_item_id'],'Delete',array("onclick"=>"return confirm('Are you sure?')"));?></td>
                                                    </tr>
                                            <?php } ?>
                                            
                                        </tbody>
                                       <?php }else{ echo '<b>There is no "Signature Item"</b>';
                                                    echo anchor('admin/restaurent/add_signature_item_by_a_restaurent/'.$this->uri->segment(4),'Add Signature');
                                       } ?> 
                                    </table>
                                </div>
                                <!-- /.table-responsive -->
