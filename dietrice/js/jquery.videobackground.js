...
(function ($, document, window) {
	...
	function play(that) {
		...
		if (video.paused) {
			video.play();
			controller.toggleClass('glyphicon glyphicon-pause glyphicon glyphicon-play').text(that.settings.controlText[1]);
		} else {
			if (video.ended) {
				video.play();
				controller.toggleClass('glyphicon glyphicon-pause glyphicon glyphicon-play').text(that.settings.controlText[1]);
				...
				video.pause();
				controller.toggleClass('glyphicon glyphicon-pause glyphicon glyphicon-play').text(that.settings.controlText[0]);
			}
		}
	}
	/*
	...
	function mute(that) {
		...
		if (video.volume === 0) {
			video.volume = 0;
			controller.toggleClass('glyphicon glyphicon-volume-up glyphicon glyphicon-volume-off').text(that.settings.controlText[2]);
		} else {
			video.volume = 0;
			controller.toggleClass('glyphicon glyphicon-volume-up glyphicon glyphicon-volume-off').text(that.settings.controlText[3]);
		}
	}
	/*
				...
				if (document.createElement('video').canPlayType) {
					...
					if (!that.settings.initialised) {
						...
						that.controls = $('<ul class="ui-video-background-controls"><li class="ui-video-background-play">'
							+ '<a class="glyphicon glyphicon-pause" href="#">' + that.settings.controlText[1] + '</a>'
							+ '</li><li class="ui-video-background-mute">'
							+ '<a class="glyphicon glyphicon-volume-up" href="#">' + that.settings.controlText[2] + '</a>'
							+ '</li></ul>');
						/*
